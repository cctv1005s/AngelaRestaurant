...
describe('菜单api测试', () => {
    ...
    it('菜单oneType接口测试', function* () {
        const r = yield request.post('/api/v1/menu/type/1');
        expect(r).to.be.a('object');
    });

    it('菜单oneDish接口测试', function* () {
        const r = yield request.post('/api/v1/menu/dish/1');
        expect(r).to.be.a('object');
    });

    it('菜单addType接口测试', function* () {
        const r = yield request.post('/api/v1/menu/type/add')
            .send({ ClassName: 'test1', ClassDescription: 'test' });
        expect(r).to.be.a('object');
    });

    it('菜单updateType接口测试', function* () {
        const r = yield request.post('/api/v1/menu/type/update')
            .send({ ID: 'test', ClassName: 'test', ClassDescription: shortid.generate() });
        expect(r).to.be.a('object');
    });
